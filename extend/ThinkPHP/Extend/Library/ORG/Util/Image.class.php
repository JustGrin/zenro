...
class Image {
    ...
    static function thumb2($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
     ...
     /**
     * 生成缩略图 按比例生成固定大小的缩略图 没有的部分白色自动填充
     * @static
     * @access public
     * @param string $image  原图
     * @param string $type 图像格式
     ...
     * @return void
     */
    static function thumb_auto($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        // 获取原图信息
        $info = Image::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
            ...
            if (function_exists("ImageCopyResampled"))
              ...
              imagecopyresized($thumbImg, $srcImg, 0, 0, 0, 0, $thumbWidth, $thumbHeight, $srcWidth, $srcHeight);
            
            //添加白边
            $final_image = imagecreatetruecolor($maxWidth, $maxHeight);
            $color = imagecolorallocate($final_image, 255, 255, 255);
            imagefill($final_image, 0, 0, $color);
            $x = round(($maxWidth - $thumbWidth) / 2);
            $y = round(($maxHeight - $thumbHeight) / 2);
            imagecopy($final_image, $thumbImg, $x, $y, 0, 0, $thumbWidth, $thumbHeight); 
            $thumbImg=$final_image;
            // 对jpeg图形设置隔行扫描
            if ('jpg' == $type || 'jpeg' == $type)
                imageinterlace($thumbImg, $interlace);
            ...
            // 生成图片
            $imageFun = 'image' . ($type == 'jpg' ? 'jpeg' : $type);
            $re=$imageFun($thumbImg,$thumbname);
            if($re===false){
                return false;
            }
            imagedestroy($thumbImg);
